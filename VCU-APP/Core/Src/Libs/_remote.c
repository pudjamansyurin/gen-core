...
uint8_t RMT_Pairing(void) {
	...
	for (uint8_t i = 0; i < 4; i++)
		aes[i] = _ByteSwap32(RMT.d.pairing_key[i]);
...
uint8_t RMT_IO_GetActive(void) { return RMT.d.active; }

uint8_t RMT_IO_GetNearby(void) { return RMT.d.nearby; }

uint32_t RMT_IO_GetTick(RMT_TICK key) { return RMT.d.tick[key]; }

uint8_t RMT_IO_GetDuration(RMT_DURATION key) { return RMT.d.duration[key]; }
