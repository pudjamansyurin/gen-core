...
void HB_RefreshSelectSet(void) {
	if (HBAR.d.session) {
		if (!_TickIn(HBAR.tick.session, MODE_SESSION_MS) || Reversed()) {
			HBAR.d.session = 0;
...
void HB_RefreshSein(void) {
	...
	if (!_TickIn(HBAR.tick.sein, 250)) {
		if (HBAR.d.pin[HBP_SEIN_L] || HBAR.d.pin[HBP_SEIN_R])
			HBAR.tick.sein = _GetTickMS();
...
static void RunSet(void) {
	...
	if (m == HBM_TRIP) {
		if (mTrip != HBMS_TRIP_ODO)
			if (HBAR.tim[HBP_SET].time > MODE_RESET_MS)
				HB_EE_Trip(mTrip, &meter);
	} else {
		if (HBAR.d.sub[m] < (HB_SubMax(m) - 1))
			mode = HBAR.d.sub[m] + 1;
		HB_EE_Sub(m, &mode);
	}
}
...
static uint8_t DefferMode(void) {
	return VCU.d.vehicle > VEHICLE_NORMAL;
}
