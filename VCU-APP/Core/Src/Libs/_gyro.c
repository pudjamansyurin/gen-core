...
static void GYRO_Readable(coordinate_t *gyro, motion_t *motion);
...
mems_decision_t GYRO_Decision(uint16_t sample) {
	...
	GYRO_Readable(&(mems.gyroscope), &motion);
...
static void GYRO_Readable(coordinate_t *gyro, motion_t *motion) {
	...
	mo.yaw = sqrt(pow(gyro->x, 2) + pow(gyro->y, 2));
	mo.roll = sqrt(pow(gyro->x, 2) + pow(gyro->z, 2));
	mo.pitch = sqrt(pow(gyro->y, 2) + pow(gyro->z, 2));

	motion->yaw = mo.yaw == 0 ? 0 : RAD2DEG(atan(gyro->z / mo.yaw));
	motion->roll = mo.roll == 0 ? 0 : RAD2DEG(atan(gyro->y / mo.roll));
	motion->pitch = mo.pitch == 0 ? 0 : RAD2DEG(atan(gyro->x / mo.pitch));
}
