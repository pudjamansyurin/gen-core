...
static uint8_t SyncedSpeedMax(void) {
  MCU.synced.rpm_max =
      !MCU.set.rpm_max || (MCU.d.par.rpm_max == MCU.set.par.rpm_max);
...
static uint8_t SyncedTemplates(void) {
  MCU.synced.template =
      !MCU.set.template ||
      (memcmp(MCU.d.par.tpl, MCU.set.par.tpl, sizeof(MCU.d.par.tpl)) == 0);
...
static void SetTemplatesCAN(uint8_t write) {
  for (uint8_t m = 0; m < HBAR_M_DRIVE_MAX; m++) {
    ...
    MCU_TX_Template(tplAddr[m].torque_max, write,
                    write ? MCU.set.par.tpl[m].torque_max * 10 : 0);
    //		MCU_TX_Template(tplAddr[m].rbs_switch, write, write ?
    // MCU.set.par.tpl[m].rbs_switch : 0);
  }
}
