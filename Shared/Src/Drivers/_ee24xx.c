...
uint8_t EEPROM24XX_IsConnected(void) {
  return (HAL_I2C_IsDeviceReady(i2c, DevAddress, 2, 1000) == HAL_OK);
}
...
uint8_t EEPROM24XX_Load(uint16_t Address, void *data, size_t size_of_data) {
  return (HAL_I2C_Mem_Read(i2c, DevAddress, Address, I2C_MEMADD_SIZE_16BIT, (uint8_t*) data, size_of_data, 100) == HAL_OK);
}
