...
/* Public variables ---------------------------------------------------------*/
sim_t SIM = {.state = SIM_STATE_DOWN,
...
uint8_t Simcom_ReadNewSMS(char *buf, uint8_t buflen) {
  ...
  //		at_cpms_t param;
  //
  //		for(uint8_t i = 0; i < sizeof(at_cpms_t) / sizeof(at_cpms_mem_t);
  //i++) 			strcpy(param.mem[i].storage, "SM"); 		res = AT_StorageMessageSMS(ATW,
  //&param);
  //	}
  //	// List SMS
  //	if (res == SIM_OK) {
...
#if (!BOOTLOADER)
  VCU.SetEvent(hard ? EVG_NET_HARD_RESET : EVG_NET_SOFT_RESET, 1);
#endif
    ...
    if (Simcom_Resp(reply, NULL) || Simcom_Resp(SIM_RSP_ERROR, NULL) ||
      ...
      // Handle failure
        ...
        if (Simcom_Resp(SIM_RSP_READY, NULL) &&
          ...
          SIM.state = SIM_STATE_READY;
#if (!BOOTLOADER)
          VCU.SetEvent(EVG_NET_SOFT_RESET, 1);
#endif
        }
...
static void SetStatePdpOn(SIM_RESULT *res) {
  ...
  *res = AT_BearerInitialize();
  ...
  else if (SIM.state == SIM_STATE_PDP_ON)
    SIM.state = SIM_STATE_GPRS_ON;
}
...
static void SetStatePdpOn(SIM_RESULT *res) {
  ...
  if (*res == SIM_OK) {
    AT_ConnectionStatus(&(SIM.ipstatus));
    if (SIM.ipstatus == CIPSTAT_IP_CONFIG ||
        SIM.ipstatus == CIPSTAT_IP_GPRSACT) {
      at_cifsr_t param;
...
static void SetStateInternetOn(SIM_RESULT *res, uint32_t tick,
  ...
  AT_ConnectionStatus(&(SIM.ipstatus));
  if (*res == SIM_OK && (SIM.ipstatus != CIPSTAT_CONNECT_OK ||
                         SIM.ipstatus != CIPSTAT_CONNECTING)) {
    at_cipstart_t param = {
