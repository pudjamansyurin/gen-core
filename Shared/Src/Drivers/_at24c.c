 ...
 * --------------------------------------------*/
static uint8_t Load(uint16_t addr, uint8_t* data, uint16_t n) {
  return (HAL_I2C_Mem_Read(pi2c, DEV_ADDR(DEV_ID), addr, I2C_MEMADD_SIZE_16BIT,
                           data, n, 100) == HAL_OK);
}

static uint8_t Save(uint16_t addr, const uint8_t* data, uint16_t n) {
  if (HAL_I2C_Mem_Write(pi2c, DEV_ADDR(DEV_ID), addr, I2C_MEMADD_SIZE_16BIT,
                        (uint8_t*)data, n, 100) == HAL_OK) {
    delayMs(15);
    return 1;
  }
  return 0;
