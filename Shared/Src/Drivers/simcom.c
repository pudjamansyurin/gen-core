 ...
 * --------------------------------------------*/
static sim_t SIM = {
    .response = NULL, .puart = &huart1, .pdma = &hdma_usart1_rx};
...
uint8_t SIM_BatSufficient(void) { return BAT_ScanVoltage() > SIM_BAT_MIN_MV; }
...
char* SIM_Resp(const char* keyword, const char* from) {
  ...
  if (start == NULL) start = &SIM_UART_RX[0];
...
uint8_t SIM_FetchTime(timestamp_t* ts) {
  ...
  if (SIMSta_SetState(SIM_STATE_READY, 0))
...
uint8_t SIM_SendUSSD(const char* ussd, char* buf, uint8_t buflen) {
  ...
  if (!SIMSta_SetState(SIM_STATE_NETWORK_ON, 0)) return 0;
...
uint8_t SIM_Upload(const void* payload, uint16_t size) {
  ...
  if (SIMSta_IO_Ip() != SIM_IP_CONNECT_OK &&
      SIMSta_IO_State() < SIM_STATE_SERVER_ON)
    return 0;
  ...
  if (res == SIM_OK) res = CmdRaw((char*)payload, size, SIM_RSP_ACCEPT, 30000);
  SIM_Unlock();
...
uint8_t SIM_GotResponse(uint32_t timeout) {
  ...
  do {
    ...
    delayMs(10);
  } while (ptr == NULL && tickIn(tick, timeout));
  ...
  while (1) {
    if (SIM_Resp(reply, NULL) || SIM_Resp(SIM_RSP_ERROR, NULL) ||
      ...
      // Handle failure
        ...
        if (SIM_Resp(SIM_RSP_READY, NULL) &&
            (SIMSta_IO_State() >= SIM_STATE_READY)) {
